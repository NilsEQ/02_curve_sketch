...
namespace cgp
{
	...
	{
		skeleton_drawable();
		skeleton_drawable(buffer<affine_rt> const& skeleton, buffer<int> const& parent_index, buffer<int> const& bodyline);
		void clear();
		void update(buffer<affine_rt> const& skeleton, buffer<int> const& parent_index, buffer<int> const& bodyline);

		float size_frame = 0.05f;
		float size_sphere = 0.01f;
		bool display_segments = true;
		bool display_joint_frame = false;
		bool display_joint_sphere = true;

		segments_drawable segments;
		segments_drawable segments_body_line;
...
namespace cgp
{
	...
	void draw(skeleton_drawable const& skeleton, SCENE const& scene)
	{
		if (skeleton.display_segments) {
			draw(skeleton.segments_body_line, scene);
